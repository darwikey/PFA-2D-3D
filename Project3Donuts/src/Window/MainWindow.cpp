...
{
    //Load the ini file to setup the window with the persistent parameters.
    ...
    //Toggle the translate button to have a default TransformWidget selected
    ui->actionTranslate->toggle();

    //Setup the object list with the current scene.
    QStringListModel* modelList = Scene::getScene()->getListObjects();
    ui->listView->setModel(modelList);
    ui->listView->setEditTriggers(QAbstractItemView::NoEditTriggers);

    //For future developers : this is the line to allow multiple selection
    //ui->listView->setSelectionMode(QAbstractItemView::MultiSelection );


    //Setting up sub windows.
    win_about = new QMainWindow();
    win_about->setWindowFlags(Qt::WindowStaysOnTopHint);
    win_about->setWindowModality (Qt::ApplicationModal); //ApplicationModal force the user to close the window to access the rest of the program.
    ...
    //Connect the setting window to the checkSettings slot, so that whenever we are done with the settings window, we change the mainwindow parameters.
...
{
    //Get the list of selected object in the list
...
{
    //setup a new splitter with corresponding parameters and delete the old one.
