...
std::unique_ptr<QImage>AutostereogramAlgorithm2::depthmapToAutostereogram(const QImage& fImage, int fDPI, enum Texture fTextureStyle, char * fTexture) {
	...
	std::vector<float> floatDepthmap = this->getDepth(fImage);
	...
	/* Linking */
	std::vector<int> sameL(maxwidth);
	std::vector<int> sameR(maxwidth);

	/* Coloration */
	std::vector<int> pixR(maxwidth);
	std::vector<int> pixG(maxwidth);
	std::vector<int> pixB(maxwidth);
